<template>
    ...
    <h2><span class="parent-header">ＲＰＧの歩行グラフィック　＞　</span>盤ウォーキング・遠ざかるグリッド吸着</h2>
    <section class="sec-2">
        <br/>
    ...
    <br/>
    <h3><span class="parent-header-lights-out">ＲＰＧの歩行グラフィック　＞　</span><span class="parent-header">盤ウォーキング・遠ざかるグリッド吸着　＞　</span>ソースコード</h3>
    <section class="sec-3">
        <source-link
...
<script setup lang="ts">
    ...
    const boardHome1Stlye = computed<CompatibleStyleValue>(() => {
        return {
            width: `${board1FileNum.value * tileBoard1TileWidth}px`,
            height: `${board1RankNum.value * tileBoard1TileHeight}px`,
        };
    });
    ...
    >(() => {
        return (i:number)=>{
            ...
            const homeLeft = (i % board1FileNum.value) * tileBoard1TileWidth;
            const homeTop = Math.floor(i / board1FileNum.value) * tileBoard1TileHeight;
    ...
    function gameLoopStart() : void {
        const update = () => {
            board1MotionWait.value -= 1;    // モーション・タイマー

            if (board1MotionWait.value==0) {
                // モーションのクリアー
                board1Motion.value["goToRight"] = 0;
                board1Motion.value["goToBottom"] = 0;
            }
            
            // ++++++++++++++++++++++++++++++
            // + キー入力をモーションに変換 +
            // ++++++++++++++++++++++++++++++
            if (board1MotionWait.value<=0) {   // ウェイトが無ければ、入力を受け付ける。
                ...
                if (player1Input.ArrowUp) {    // 上
                    board1Motion.value["goToBottom"] = commonSpriteMotionUp;
                }

                if (player1Input.ArrowDown) {    // 下
                    board1Motion.value["goToBottom"] = commonSpriteMotionDown;
                }

                // モーションの入力があれば、ウェイトを入れる。
                if (board1Motion.value["goToRight"]!=0 || board1Motion.value["goToBottom"]!=0) {
                    board1MotionWait.value = board1AnimationWalkingFrames;
                }
            }
