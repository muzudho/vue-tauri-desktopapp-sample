<template>
    ...
    <h2><span class="parent-header">ＲＰＧの歩行グラフィック　＞　</span>不動タイルへ投影・境界のある像、盤の端歩き</h2>
    <section class="sec-2">
        <br/>
    ...
    <br/>
    <h3><span class="parent-header-lights-out">ＲＰＧの歩行グラフィック　＞　</span><span class="parent-header">不動タイルへ投影・境界のある像、盤の端歩き　＞　</span>ソースコード</h3>
    <section class="sec-3">
        <source-link
...
<script setup lang="ts">
    ...
    const player1File = computed<number>(()=>{
        return Math.round(player1Left.value / tileBoard1TileWidth);
    });
    const player1Rank = computed<number>(()=>{
        return Math.round(player1Top.value / tileBoard1TileHeight);
    });
