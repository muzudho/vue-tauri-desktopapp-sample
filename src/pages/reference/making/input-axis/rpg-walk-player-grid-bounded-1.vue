<template>
    ...
    <h2><span class="parent-header">ＲＰＧの歩行グラフィック　＞　</span>自機ウォーキング・境界のあるグリッド吸着</h2>
    <section class="sec-2">
        <br/>
    ...
    <br/>
    <h3><span class="parent-header-lights-out">ＲＰＧの歩行グラフィック　＞　</span><span class="parent-header">自機ウォーキング・境界のあるグリッド吸着　＞　</span>ソースコード</h3>
    <section class="sec-3">
        <source-link
            pagePath="/reference/making/input-axis/rpg-walk-player-grid-bounded-1"/>
    </section>
</template>
    ...
    function gameLoopStart() : void {
        const update = () => {
            ...
            if (player1MotionWait.value<=0) {   // ウェイトが無ければ、入力を受け付ける。
                ...
                // 位置のリセット
                if (player1Input[" "]) {
                    player1Top.value = 1 * tileBoard1TileHeight;  // 自機
            ...
            if (player1Motion.value["goToRight"]==1) {    // 右
                ...
                if (player1Left.value < (board1FileNum.value - 1) * tileBoard1TileWidth) {    // 境界チェック
            ...
            if (player1Motion.value["goToBottom"]==-1) {    // 上
                ...
                if (player1Top.value < (board1RankNum.value - 1) * tileBoard1TileHeight) {
                    player1Top.value += player1Speed.value;
                }
            }
